...
- **Ending pace:** after gameplay/economy changes, run `npm run balance:endings` to report simulated time-to-ending for each capstone path (headless greedy player in `js/balance-sim.js`). CI runs this via `.github/workflows/balance-endings.yml`.
