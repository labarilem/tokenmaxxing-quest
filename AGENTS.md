...
- Game progress is stored in `localStorage` under the key `tokenmaxxing-quest.save.v1`. To test from a clean slate, clear that key (or use a fresh browser profile) — otherwise a prior save will load on start.
